/**
 * Build the browser version of newdb
 */
...
child_process.exec('npm install', { cwd: __dirname }, function (err, stdout, stderr) {
  ...
  , function (cb) {
    ...
    b.add(srcPath);
    b.bundle({ standalone: 'Newdb' }, function (err, out) {
      if (err) { return cb(err); }
      fs.writeFile(path.join(__dirname, 'out/newdb.js'), out, 'utf8', function (err) {
        if (err) {
          return cb(err);
        } else {
          return cb(null, out);
  ...
  , function (out, cb) {
      ...
      fs.writeFile(path.join(__dirname, 'out/newdb.min.js'), compressedCode.code, 'utf8', cb);
  }
  ], function (err) {
    if (err) {
